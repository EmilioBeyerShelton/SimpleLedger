export interface ElectronLedgerBridge {
  ...
  /** Writes raw uploaded .sqlite3 bytes to a throwaway temp file, reads it via the normal DB-reading code path, and returns the resulting LedgerData as a JSON-serialized string. Used only for the manual "upload backup" flow. */
  dbReadFromBytes(bytes: Uint8Array): Promise<{ ok: true; data: string } | { ok: false; error: string }>;
